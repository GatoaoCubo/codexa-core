...
**Required**:
- `$scene_grid` (object): 9-scene grid from Phase 1
  - Structure: `{ scene_1: {angle, purpose}, ..., scene_9: {...} }`
- `$photography_style` (enum): Style preset (commercial, lifestyle, etc.)

**Optional**:
- `$technical_overrides` (object): Manual camera/lighting overrides

**Validation**:
- ✅ All 9 scenes must be defined
- ✅ No impossible combinations (f/1.2 + full focus)
...
**Primary**:
- `$camera_specs` (object): Complete camera config per scene
  - Structure: `{ scene_1: {focal, aperture, shutter, iso}, ..., scene_9: {...} }`

**Secondary**:
- `$lighting_design` (object): Lighting setup per scene
  - Structure: `{ scene_1: {type, direction, temp}, ..., scene_9: {...} }`

**Consumed By**: Phase 3 (Prompt Generator HOP)
