#!/usr/bin/env python3
"""
ISO Vectorstore Validator - marca_agent
Validates conformance with pesquisa_agent v3.1.0 patterns
...
class ISOValidator:
    """Validates ISO vectorstore compliance for marca_agent"""
    ...
    def validate_hops_exist(self) -> bool:
        """Validates HOP files exist (marca_agent has 2 HOPs)"""
        required_hops = [
            "13_HOP_brand_identity.md",
            "14_HOP_main_agent.md",
        ]
        ...
        self.info.append(f"[OK] HOPs present ({found_count}/{len(required_hops)})")
        return all_valid

    def validate_reference_docs(self) -> bool:
        """Validates reference documentation files (15-18)"""
        reference_files = [
            "15_brand_archetypes.md",
            "16_positioning_framework.md",
            "17_visual_identity.md",
            "18_messaging_guide.md",
        ]
        ...
        for filename in reference_files:
            ...
            if not file_path.exists():
                self.errors.append(f"[FAIL] Reference file missing: {filename}")
                all_valid = False
            else:
                found_count += 1

        self.info.append(f"[OK] Reference docs ({found_count}/{len(reference_files)})")
        return all_valid
        ...
        for file in files:
            if file.suffix in ['.md', '.json']:
                try:
                    ...
                    # Heuristic: files >2000 lines or >30000 chars may violate OPOP
                    # marca_agent has larger reference docs, so we use 30000 threshold
                    if line_count > 2000 or char_count > 30000:
    ...
    def validate_output_template(self) -> bool:
        """Validates output template exists with 32 blocks structure"""
        template = self.iso_path / "09_output_template.md"

        if not template.exists():
            self.errors.append("[FAIL] 09_output_template.md NOT FOUND")
            return False

        try:
            content = template.read_text(encoding='utf-8')

            # Check for 32 blocks mention or section structure
            if "32" in content or "SECTION" in content:
                self.info.append("[OK] Output template has section structure")
            else:
                self.warnings.append("[WARN] Output template may need section structure")

            return True

        except Exception as e:
            self.errors.append(f"[FAIL] Error reading output template: {e}")
            return False
    ...
    def validate_system_instructions(self) -> bool:
        ...
        try:
            ...
            # Check version
            if "v3.1.0" in content:
                self.info.append("[OK] SYSTEM_INSTRUCTIONS is v3.1.0")
            elif "v3.0.0" in content or "v3." in content:
                self.warnings.append("[WARN] SYSTEM_INSTRUCTIONS is v3.0.0 (v3.1.0 recommended)")
            else:
                self.warnings.append("[WARN] SYSTEM_INSTRUCTIONS version may be outdated")

            # Check 32 blocks mentioned
            if "32" in content and "block" in content.lower():
                self.info.append("[OK] SYSTEM_INSTRUCTIONS mentions 32 blocks")
            else:
                self.warnings.append("[WARN] SYSTEM_INSTRUCTIONS should mention 32 blocks")
    ...
    def validate_brand_validator(self) -> bool:
        """Validates brand_validator.py exists in src/"""
        parent = self.iso_path.parent
        validator = parent / "src" / "brand_validator.py"

        if not validator.exists():
            self.errors.append("[FAIL] src/brand_validator.py NOT FOUND")
            return False

        self.info.append("[OK] brand_validator.py exists in src/")
        return True

    def validate_file_numbering(self) -> bool:
        """Validates file numbering consistency (00-18)"""
        expected_prefixes = [
            "00_", "01_", "02_", "03_", "04_", "05_", "06_", "07_", "08_", "09_", "10_",
            "11_", "12_", "13_", "14_", "15_", "16_", "17_", "18_"
        ]

        files = sorted([f.name for f in self.iso_path.glob("*") if f.is_file()])
        numbered_files = [f for f in files if f[:3] in expected_prefixes]

        self.info.append(f"[OK] Found {len(numbered_files)} numbered files (target: 19-20)")
    ...
    def validate_core_docs(self) -> bool:
        ...
        self.info.append(f"[OK] Core docs ({found_count}/{len(core_files)})")
        return all_valid
    ...
    def run_all_validations(self) -> Tuple[int, int]:
        ...
        """
        print("\n" + "="*60)
        print("ISO VECTORSTORE VALIDATION - marca_agent v3.1.0")
        print("="*60 + "\n")
        ...
        validations = [
            ...
            ("HOPs (13-14)", self.validate_hops_exist),
            ("Reference Docs (15-18)", self.validate_reference_docs),
            ("ADW Orchestrator", self.validate_adw_orchestrator),
            ("Output Template", self.validate_output_template),
            ("OPOP Compliance", self.validate_opop_compliance),
            ("SYSTEM_INSTRUCTIONS", self.validate_system_instructions),
            ("Brand Validator (src/)", self.validate_brand_validator),
