...
# Looking for adw_modules in the adws root directory (2 levels up)
adws_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
sys.path.insert(0, os.path.join(adws_root, "adw_modules"))
...
def main(
    ...
    if SCOUT_AVAILABLE:
        try:
            console.print("[cyan][SCOUT][/cyan] Loading repository context...")
            scout_context = prepare_scout_context_sync(working_dir=worktree_path if os.path.exists(worktree_path) else ".")
            if scout_context.get('success'):
                stats = get_repository_stats(scout_context)
                console.print(f"[green][SCOUT][/green] Repository indexed: {stats.get('total_files', 0)} files")

                # Find build-related files for context
                build_related_files = extract_related_files(scout_context, pattern="*build*")
                test_files = extract_related_files(scout_context, pattern="*test*")
                console.print(f"[green][SCOUT][/green] Found {len(build_related_files)} build files, {len(test_files)} test files")
            else:
                console.print(f"[yellow][SCOUT][/yellow] Context failed, continuing without repository context")
