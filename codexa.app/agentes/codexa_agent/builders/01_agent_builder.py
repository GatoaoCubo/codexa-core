    ...
    if SCOUT_AVAILABLE:
        try:
            ...
            if scout_context.get('success'):
                ...
                # Find task-related files for better planning
                plan_files = extract_related_files(scout_context, pattern="*plan*")
                impl_files = extract_related_files(scout_context, pattern="*implement*")
                task_files = extract_related_files(scout_context, pattern="*task*")
                ...
                console.print(f"[green][SCOUT][/green] Found {len(plan_files)} plan, {len(impl_files)} impl, {len(task_files)} task files")
            else:
