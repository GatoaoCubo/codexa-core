...
def generate_sql_with_openai(query_text: str, schema_info: Dict[str, Any]) -> str:
    """
    Generate SQL query using OpenAI API (with LLM caching)
    """
    try:
        # Check cache first
        cache = get_cache()
        if cache:
            cached_sql = cache.get(query_text, schema_info, "gpt-4.1-2025-04-14")
            if cached_sql:
                return cached_sql
...
SQL Query:"""
        ...
        # Store in cache
        if cache:
            cache.set(query_text, schema_info, "gpt-4.1-2025-04-14", sql)
...
def generate_sql_with_anthropic(query_text: str, schema_info: Dict[str, Any]) -> str:
    """
    Generate SQL query using Anthropic API (with LLM caching)
    """
    try:
        # Check cache first
        cache = get_cache()
        if cache:
            cached_sql = cache.get(query_text, schema_info, "claude-sonnet-4-0")
            if cached_sql:
                return cached_sql
...
SQL Query:"""
        ...
        # Store in cache
        if cache:
            cache.set(query_text, schema_info, "claude-sonnet-4-0", sql)
