...
This script scans markdown files for path references and validates them against
the current directory structure. It detects obsolete -standalone naming patterns
and ensures all path references are coordinated and traceable.

Usage:
    python scripts/validate_taxonomy.py --scan-all
    python scripts/validate_taxonomy.py --file path/to/file.md
    python scripts/validate_taxonomy.py --scan-all --fix (automated replacement)
    python scripts/validate_taxonomy.py --scan-all --report output.txt
"""
...
def main():
    ...
    # Determine repository root (assuming script is in scripts/)
